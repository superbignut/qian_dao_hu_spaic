...
class nearest_online_STDP(Base_STDP):
    ...
    def __init__(self, trainable=None, *args, **kwargs):
        super(nearest_online_STDP, self).__init__(trainable=trainable) #  nearest_online_stdp 的trace并没有进行衰减，但是full的有衰减，很有可能也是因为，full的不衰减会积累的很大
        ...
        self._constant_variables['trace_pre'] = kwargs.get('trace_pre', np.exp(-1/20)) # trace衰减初始化
        ...
        self.w_norm = 6.4 # 28 * 28 = 784 微调的时候 是不是要小一点 比较好
        # self.w_norm = 5.6 # 取一半吧 # 我发现这里一改之后，测试的代码就出问题， 按道理说这里 在测试的时候不进行更新吧
        self.trainable = trainable

    def nearest_online_stdp_weightupdate(self, dw, weight, reward): # 给stdp加了reward
        '''
        ...
        with torch.no_grad():
            # return weight.clamp_(0.0, 1.0)
            # print(" reward is:", reward)
            # weight = weight + dw
            #last_w = weight.clone()
            # torch.save(weight, "a.pth")
            # tem_w = weight.clone()
            weight.add_(dw) # 这里以后得看看为啥，测试的时候也会被歌更新 还是说只是 一个run_time内 更新，结束后再变回去？？？ 或者根本就不变
            # print("yes")
            ...
            weight[...] = (self.w_norm * torch.div(weight, torch.sum(torch.abs(weight), 1, keepdim=True))) # 先归一化 再乘上 norm 因子 再clamp
            # 这里的dim=1 是要把 100 * 784 的权重 sunm为 100 个和 相当于对每一个神经元的784 的权重进行归一化 又乘了78.4 因此 相当于把所有的 权重又进行了放大， 
            # 大致相当于 整体缩小10倍
            # 100 * 784， 更新 每一个 输出结果对 输入的全部 维度的 归一化 # 这里也是论文中提到的，在竞争之中引入平均归一化
            # 对第一个维度求和的话是 就是一个输入和与它相连的所有输出 正常的归一化
            weight.clamp_(0.0, 1.0)
            # 
            # 
            # torch.save(weight, "b.pth")
            # print(tem_w == weight)
            return weight
    ...
    def build(self, backend):
        ...
        for conn in self.trainable_connections.values():
            ...
            # print(pre_name, type(pre_name)) # autoname1<net>_input<nod>:{O} <class 'str'>
            ...
            #
            # output_trace_s = (self.output_trace * self.trace_decay) # 衰减项再下面的 op_to_backend 中并没有出现，有出现名字是pre post
            ...
            self.op_to_backend('input_trace_s', 'var_mult', [input_trace_name, 'trace_pre']) # 衰减
            self.op_to_backend('input_temp', 'minus', ['spike', pre_name]) # spike = 1是常数  pre_name 表示 input_spike
            self.op_to_backend(input_trace_name, 'var_linear', ['input_temp', 'input_trace_s', pre_name]) # a * b + c  input_temp * input_trace_s + pre_name
            ...
            self.op_to_backend('pre_post_temp', 'mat_mult_pre', [post_name, input_trace_name+'[updated]']) # 矩阵相乘 
            ...
            self.op_to_backend(dw_name, 'minus', ['pre_post', 'post_pre']) # 相减
            self.op_to_backend(weight_name, self.nearest_online_stdp_weightupdate, [dw_name, weight_name,'Output_Reward[updated]']) # 给stdp加了reward
